...
export const ProjectMap: React.FC<{ className?: string }> = ({ className }) => {
  ...
  useEffect(() => {
    ...
    const getDynamicStyles = () => {
      ...
      return {
        hover: sat
          ? { color: '#3b82f6', fillColor: 'transparent', weight: 2, fillOpacity: 0 }
          : { color: '#0f172a', fillColor: '#93c5fd', weight: 2, fillOpacity: 0.35 },
        selected: sat
          ? { color: '#ef4444', fillColor: 'transparent', weight: 2.5, fillOpacity: 0 }
          : { color: '#dc2626', fillColor: '#fca5a5', weight: 3, fillOpacity: 0.22 },
        districtBase: sat ? satelliteDistrictStyle : standardDistrictStyle,
        nestedBase: sat
          ? { color: '#ffffff', fillColor: 'transparent', weight: 1.2, fillOpacity: 0 }
          : { color: '#0f172a', fillColor: '#94a3b8', weight: 1, fillOpacity: 0.55 },
        wardBase: sat
          ? { color: '#ffffff', fillColor: 'transparent', weight: 1, fillOpacity: 0 }
          : { color: '#0f172a', fillColor: '#cbd5e1', weight: 1, fillOpacity: 0.45 },
      };
    };
    ...
    const run = async () => {
      ...
      async function loadDistrict(dFeature: GeoJSON.Feature) {
        ...
        districtLayerRef.current = L.geoJSON(districtData as GeoJSON.GeoJsonObject, {
          ...
          onEachFeature(lFeature: GeoJSON.Feature, lLayer) {
            ...
            lPath.on({
              ...
              mouseout: () => {
                if (selectedLayerRef.current !== lPath) districtLayerRef.current?.resetStyle(lPath);
              },
              click: () => {
        ...
        if (map.getZoom() <= ZOOM_DISTRICT_MAX)
        ...
        localBodyLayerRef.current = L.geoJSON(localData as GeoJSON.GeoJsonObject, {
          ...
          onEachFeature(f: GeoJSON.Feature, l) {
            ...
            path.on({
              ...
              mouseout: () => {
                if (selectedLayerRef.current !== path) localBodyLayerRef.current?.resetStyle(path);
              },
              click: () => {
  ...
  return (
    <div className={cn('project-map flex h-full min-h-[420px] w-full overflow-hidden rounded-xl border border-border-main bg-white shadow-sm', className)}>
      ...
      `}</style>
      <div ref={mapElRef} className="relative z-0 min-h-0 min-w-0 flex-1" role="application" aria-label="Nepal map view" />
    </div>
  );
};