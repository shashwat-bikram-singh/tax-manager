...
export default function SummaryReport() {
    ...
    const handleOpenDetails = (item: Property) => {
    ...
    const handleViewFile = async (propertyId: number) => {
        if (!propertyId) return alert("Cannot view file: Invalid record ID");
        try {
            const response = await axiosInstance.get<ApiResponseViewFile>(`/api/view-receipt?id=${propertyId}`);
            if (response?.data?.url) { setSelectedFileUrl(response.data.url); setIsModalOpen(true); }
            else { alert(t("payment.NoFileUrlFoundForThisRecord")); }
        } catch (error) { console.error(error); alert(t("payment.FailedToFetchFile")); }
    };
    ...
    const handleDownload = async () => {
        ...
        } catch (error) { window.open(selectedFileUrl, "_blank"); }
    };

    const handleDelete = async () => {
        if (propertyToDelete && propertyToDelete.id) {
            try { await deleteProperty.mutateAsync(propertyToDelete.id); toast.success("Property deleted successfully"); setDeleteDialogOpen(false); setPropertyToDelete(null); }
            catch (err) { toast.error("Failed to delete property"); }
        }
    };

    if (isLoadingItems) {
        return (<div className="p-8 flex justify-center items-center min-h-[400px]"><div className="animate-spin rounded-full h-8 w-8 border-b-2 border-blue-600"></div></div>);
    }

    // Reusable detail row component
    const DetailRow = ({ label, value, isBadge }: { label: string, value: string | number | null | undefined, isBadge?: boolean }) => (
        <div className="flex flex-col gap-1">
            <span className="text-[10px] font-bold uppercase tracking-widest text-slate-400">{label}</span>
            <div className="min-h-[24px] flex items-center">
                {isBadge && value ? getStatusBadge(String(value)) : (
                    <span className="text-sm font-medium text-slate-800">
                        {value !== null && value !== undefined && value !== "" ? (typeof value === 'number' ? value.toLocaleString() : value) : "-"}
                    </span>
                )}
            </div>
        </div>
        ...
        <div className="-mt-5 md:p-1 max-w-9xl mx-auto space-y-4">
            ...
            <div className="hidden md:block rounded-xl border border-slate-200 bg-white shadow-sm overflow-hidden">
                <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 bg-white p-4 rounded-xl border border-slate-200 shadow-sm">
                    <div className="flex items-center gap-3">
                        <Button variant="outline" size="sm" className="text-slate-500 border-slate-200 hover:bg-slate-50 p-2 h-9 w-9 shrink-0" onClick={() => setSearchOpen(!searchOpen)}>
                            <Search className="h-4 w-4" />
                        </Button>
                        {searchOpen && (
                            <div className="relative w-full sm:w-80 group">
                                <Input ref={searchInputRef} type="search" placeholder="Search" className="pl-9 w-full bg-slate-50 border-2 focus:bg-white focus:ring-blue-500 focus:border-blue-500 transition-all" value={searchTerm} onChange={(e) => { setSearchTerm(e.target.value); setCurrentPage(1); }} />
                            </div>
                        )}
                        <DropdownMenu>
                            <DropdownMenuTrigger asChild>
                                <Button variant="outline" className="text-slate-600 border-slate-200 hover:bg-slate-50 gap-2 shrink-0"><Settings2 /></Button>
                            </DropdownMenuTrigger>
                            <DropdownMenuContent align="end" className="w-40">
                                <DropdownMenuLabel>Toggle Columns</DropdownMenuLabel>
                                <DropdownMenuSeparator />
                                {Object.entries(columnVisibility).map(([key, visible]) => (
                                    <DropdownMenuCheckboxItem key={key} checked={visible} onCheckedChange={() => setColumnVisibility({ ...columnVisibility, [key]: !visible })}>
                ...
                <Table>
                    <TableHeader className="bg-slate-50 border-b border-slate-200">
                        <TableRow className="hover:bg-slate-50/50">
                            {columnVisibility.id && (
                                <TableHead className="w-16 text-center text-[11px] font-bold text-slate-500 uppercase tracking-wider py-2 cursor-pointer hover:text-slate-700 select-none" onClick={() => handleSort("id")}>
                                    <div className="flex items-center justify-center gap-1">ID {sortConfig.key === "id" ? (sortConfig.direction === "asc" ? <ArrowUp className="w-3 h-3 text-blue-600" /> : <ArrowDown className="w-3 h-3 text-blue-600" />) : <ArrowUpDown className="w-3 h-3 opacity-40" />}</div>
                                </TableHead>
                            )}
                                    ...
                                    {columnVisibility.actions && (
                                        <TableCell className="py-2">
                                            <div className="flex items-center justify-center gap-1.5">
                                                <Button size="sm" variant="ghost" className="h-8 w-8 p-0 text-slate-500 hover:bg-slate-100 hover:text-blue-600" onClick={() => handleViewFile(item.id)} title="View Receipt"><Eye size={16} strokeWidth={2} /></Button>
                                            </div>
                                        </TableCell>
                                    )}
                                </TableRow>
            ...
            <div className="hidden sm:flex items-center justify-between px-2 pt-4">
                <div className="text-sm text-slate-500">
                    Showing <span className="font-medium text-slate-900">{filteredProperties.length === 0 ? 0 : (currentPage - 1) * entriesPerPage + 1}</span> to <span className="font-medium text-slate-900">{Math.min(currentPage * entriesPerPage, filteredProperties.length)}</span> of <span className="font-medium text-slate-900">{filteredProperties.length}</span> results
                </div>
                <div className="flex items-center space-x-6 lg:space-x-8">
                    <div className="flex items-center space-x-2">
                        ...
                        <Select value={entriesPerPage.toString()} onValueChange={(v) => { setEntriesPerPage(Number(v)); setCurrentPage(1); }}>
                            ...
                            <SelectContent side="top">{[10, 20, 30, 40, 50].map((size) => (<SelectItem key={size} value={size.toString()}>{size}</SelectItem>))}</SelectContent>
                        </Select>
                    </div>
                    <div className="flex items-center space-x-2">
                        <div className="flex w-[100px] items-center justify-center text-sm font-medium text-slate-700">Page {currentPage} of {totalPages}</div>
                ...
                <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm">
                    <div className="bg-white w-full max-w-4xl max-h-[90vh] flex flex-col rounded-2xl shadow-2xl overflow-hidden animate-in zoom-in-95 fade-in duration-200">
                        ...
                        <div className="flex items-center justify-between px-6 py-4 border-b bg-slate-50 shrink-0">
                            ...
                            <button onClick={closeDetailsModal} className="p-2 hover:bg-red-50 text-slate-400 hover:text-red-500 rounded-full transition-all"><X size={20} /></button>
                        </div>
                                ...
                                <div className="bg-white p-4 rounded-xl border border-slate-100 shadow-sm">
                                    ...
                                    <div className="space-y-2">
                                        {getDocuments(selectedProperty.document).map((doc) => (
                                            <a
                                                key={doc.Id}
                                                href={`/api/documents/${doc.FilePath}`} // Adjust this URL based on how your backend serves files
                                                target="_blank"
                                                rel="noopener noreferrer"
                                                className="flex items-center gap-3 p-3 bg-slate-50 hover:bg-blue-50 border border-slate-200 hover:border-blue-200 rounded-lg transition-colors group"
                                            >
            ...
            {isModalOpen && selectedFileUrl && (
                <div className="fixed inset-0 z-[100] flex items-center justify-center overflow-hidden p-4">
                    <div className="absolute inset-0 bg-slate-900/80 backdrop-blur-sm transition-opacity" onClick={closeModal} />
                    <div className="relative bg-white w-full max-w-6xl h-[95vh] rounded-2xl shadow-2xl flex flex-col overflow-hidden animate-in zoom-in-95 duration-200">
                        <div className="flex items-center justify-between px-6 py-3 border-b border-gray-200 bg-slate-50">
                            <div className="flex items-center gap-3">
                                <div className="bg-red-100 p-2 rounded-lg"><span className="text-red-600 font-bold text-xs">PDF</span></div>
                                <div className="overflow-hidden">
                                    <h3 className="text-sm font-bold text-slate-800 truncate max-w-[200px] md:max-w-md">{selectedFileUrl.split("/").pop()?.split("?")[0] || "Document Preview"}</h3>
                                    <p className="text-[10px] text-slate-400 italic font-medium">Secure Cloud Viewer</p>
                                </div>
                            </div>
                            <button onClick={closeModal} className="p-2 hover:bg-red-50 text-slate-400 hover:text-red-500 rounded-full transition-all"><X className="h-5 w-5" /></button>
                        </div>
                        <div className="flex-grow bg-slate-200 flex flex-col overflow-hidden relative">
                            {selectedFileUrl.toLowerCase().includes(".pdf") ? (
                                <div className="w-full h-full bg-white">
                                    <object data={selectedFileUrl} type="application/pdf" className="w-full h-full border-none shadow-inner">
                                        <div className="flex flex-col items-center justify-center h-full p-4 text-slate-600 bg-slate-50">
                                            ...
                                            <a href={selectedFileUrl} target="_blank" rel="noopener noreferrer" className="bg-blue-600 hover:bg-blue-700 text-white px-4 py-2 rounded-xl font-bold text-xs shadow transition-all">Open PDF in New Tab</a>
                                        </div>
                                    </object>
                                </div>
                            ) : (
                                <div className="w-full h-full flex items-center justify-center p-4 bg-slate-300 overflow-auto">
                                    <img src={selectedFileUrl} alt="Preview" className="max-w-full max-h-full object-contain rounded-lg shadow-xl bg-white p-1" onError={(e) => { e.currentTarget.src = "https://placehold.co/600x400?text=Preview+Not+Available"; }} />
                                </div>
                            )}
                        </div>
                        <div className="px-6 py-2 bg-white border-t border-gray-100 flex justify-between items-center">
                            <button onClick={closeModal} className="px-4 py-2 text-sm font-semibold text-slate-500 hover:bg-slate-50 rounded-lg transition">Close</button>
                            <div className="flex items-center gap-4">
                                {Role === "Admin" ? (
                                    <button type="button" onClick={handleDownload} className="flex items-center gap-2 bg-indigo-600 hover:bg-indigo-700 text-white px-6 py-2.5 rounded-xl text-sm font-bold shadow-lg shadow-indigo-100 transition-all active:scale-95">
